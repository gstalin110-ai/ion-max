...
export function CheckoutPage() {
  ...
  const [sellerPaymentMethod, setSellerPaymentMethod] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    if (savedCart) {
      try {
        ...
        // Cargar método de pago del vendedor
        if (parsedCart.length > 0) {
          const sellerId = parsedCart[0].user_id;
          loadSellerPaymentMethod(sellerId);
        }
      } catch {
        setCart([]);
      }
  ...
  const loadSellerPaymentMethod = async (sellerId: string) => {
    try {
      const { data } = await supabase
        .from("profiles")
        .select("payment_method, payment_data")
        .eq("id", sellerId)
        .single();

      if (data) {
        setSellerPaymentMethod(data);
      }
    } catch (error) {
      console.error("Error cargando método de pago del vendedor:", error);
    }
  };
  ...
  const handleCreateOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payment = await createPayment(order.id, "qr_link", total);

      setOrderId(order.id);
      setStep("confirmation");
      toast.success("Orden creada. Completa el pago usando el método del vendedor.");
    } catch (error) {
              ...
              >
                <div className="rounded-3xl border border-white/10 bg-zinc-950/80 p-8">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h2 className="text-xl font-black">Método de pago del vendedor</h2>
                  </div>

                  <form onSubmit={handleCreateOrder} className="space-y-6">
                    {!sellerPaymentMethod ? (
                      <div className="rounded-2xl border border-orange-500/20 bg-orange-500/10 p-4">
                        <p className="text-orange-400 font-black text-sm">El vendedor no ha configurado un método de pago</p>
                        <p className="text-xs text-zinc-400 mt-2">Contacta al vendedor para coordinar el pago.</p>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        {sellerPaymentMethod.payment_method === "qr" && (
                          <div className="rounded-2xl border border-white/10 bg-black/50 p-6">
                            <div className="flex items-center gap-3 mb-4">
                              <QrCode className="h-5 w-5 text-yellow-400" />
                              <p className="font-black">Pago con Código QR</p>
                            </div>
                            <div className="space-y-3">
                              <div className="flex items-center gap-2">
                                <span className="text-sm text-zinc-400">URL del QR:</span>
                                <button
                                  type="button"
                                  onClick={() => handleCopyToClipboard(sellerPaymentMethod.payment_data?.qr_code || "")}
                                  className="text-yellow-400 hover:text-yellow-300"
                                >
                                  <Copy className="h-4 w-4" />
                                </button>
                              </div>
                              <p className="text-sm text-white break-all">{sellerPaymentMethod.payment_data?.qr_code || "No configurado"}</p>
                              {sellerPaymentMethod.payment_data?.phone && (
                                <div className="pt-2 border-t border-white/10">
                                  <span className="text-sm text-zinc-400">Teléfono: {sellerPaymentMethod.payment_data.phone}</span>
                                </div>
                              )}
                            </div>
                          </div>
                        ...
                        {sellerPaymentMethod.payment_method === "link" && (
                          <div className="rounded-2xl border border-white/10 bg-black/50 p-6">
                            <div className="flex items-center gap-3 mb-4">
                              <LinkIcon className="h-5 w-5 text-yellow-400" />
                              <p className="font-black">Pago con Link</p>
                            </div>
                            <div className="space-y-3">
                              <div className="flex items-center gap-2">
                                <span className="text-sm text-zinc-400">Link de pago:</span>
                                <button
                                  type="button"
                                  onClick={() => handleCopyToClipboard(sellerPaymentMethod.payment_data?.payment_link || "")}
                                  className="text-yellow-400 hover:text-yellow-300"
                                >
                                  <Copy className="h-4 w-4" />
                                </button>
                              ...
                              <a
                                href={sellerPaymentMethod.payment_data?.payment_link}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-sm text-yellow-400 hover:text-yellow-300 break-all"
                              >
                                {sellerPaymentMethod.payment_data?.payment_link || "No configurado"}
                              </a>
                              {sellerPaymentMethod.payment_data?.phone && (
                                <div className="pt-2 border-t border-white/10">
                                  <span className="text-sm text-zinc-400">Teléfono: {sellerPaymentMethod.payment_data.phone}</span>
                        ...
                        <div className="rounded-2xl border border-blue-500/20 bg-blue-500/10 p-4">
                          <p className="text-blue-400 font-black text-sm">Instrucciones de pago</p>
                          <ol className="mt-2 text-xs text-zinc-400 space-y-1 list-decimal list-inside">
                            <li>Usa el método de pago mostrado arriba para enviar ${total.toFixed(2)}</li>
                            <li>Confirma el pago con el vendedor</li>
                            <li>El vendedor aprobará tu orden manualmente</li>
                          </ol>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex gap-3">
                      ...
                      <button
                        type="submit"
                        disabled={processing || !sellerPaymentMethod}
